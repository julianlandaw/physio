...
function uuidLike() {
  // Small fallback for browsers without crypto.randomUUID()
...
function loadStrategyToInputs(id) {
  ...
  if (!s) return;
  // Switch drug preset if needed
  try {
    if (s.drug && typeof window[s.drug] === 'function') {
      window[s.drug](); // sets units + pk + ke0
    }
  } catch (e) {
  ...
  // Apply PK params (in case user wants exact snapshot)
...
function ensureCompareUI() {
  ...
  addBtn.addEventListener('click', function() { captureCurrentStrategy(); });
  clearBtn.addEventListener('click', function() { clearStrategies(); });
...
function simulateStrategy(state) {
  // Returns internal mg/mL Cp and Ce arrays (not display-scaled)
  const p = state.pk;
  ...
  // Initial: amounts for x's, concentration for Ce
  ...
  // Matrix exponential for fixed dt
  const mat = math.matrix([
    [ -dt*(k10 + k12 + k13),  dt*k21,             dt*k31,           0, dt ],
    [  dt*k12,              -dt*k21,             0,                0, 0  ],
    [  dt*k13,               0,                 -dt*k31,           0, 0  ],
...
function computeMetrics(state, sim) {
  ...
  return {
    ...
    aucCp: trapz(cp, ts),
    aucCe: trapz(ce, ts),
    finalCp: cp[cp.length-1],
    finalCe: ce[ce.length-1]
  };
}
...
function plotComparisonFromCurrent() {
  ...
  if (all.length <= 1) return; // nothing to compare
  ...
  all.forEach((s, i) => {
    ...
    traces.push({
      x: sim.ts, y: sim.cp.map(v => v * yFactor),
      name: `${s.name} Cp (${unitLabel})`,
      legendgroup: s.id,
      line: { color, width: 2 }
    });
    traces.push({
      x: sim.ts, y: sim.ce.map(v => v * yFactor),
      name: `${s.name} Ce (${unitLabel})`,
      legendgroup: s.id,
      line: { color, width: 2, dash: 'dot' }
    });

    const m = computeMetrics(s, sim);
    metricsRows.push({ state: s, sim, metrics: m });
  });

  // Therapeutic bands/legend
  const layout = {
    title: { text: 'Central vs Effect-site (Comparison)', font: { family: 'Courier New, monospace', size: 24 } },
    xaxis: { title: { text: 'Time (min)', font: { family: 'Courier New, monospace', size: 18, color: '#7f7f7f' } } },
    yaxis: { title: { text: `Concentration (${unitLabel})`, font: { family: 'Courier New, monospace', size: 18, color: '#7f7f7f' } } }
  };
  ...
  legendOutsideBottom(layout, { y: -0.50, bottom: 200, fontSize: 11, itemWidth: 90 });
...
function renderCompareResults(rows, unitLabel) {
  ...
  function fmt(x, sig=3) {
    return roundToSignificantFigures(x, sig);
  }

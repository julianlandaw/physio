...
var TVhtml = document.getElementById("TVhtml");
TVhtml.innerHTML = "Volume Control (mL)";
var TVnum = document.getElementById("TV");
TVnum.value = TVslider.value/10;
...
var IHFslider = document.getElementById("IHFslider");
var IHFhtml = document.getElementById("IHFhtml");
IHFhtml.innerHTML = "Inspiratory Hold Fraction";
var IHFnum = document.getElementById("IHF");
IHFnum.value = IHFslider.value/100;
...
function dfsolve() {
    ...
    function iterate1(t, C1, C2, C3, params) { //C1 Pressure, C2 is Volume, C3 is Flow
        ...
        const ihfraction = params.IHF;
        const R = params.R;
        const C = params.C;
        const breathduration = ifraction*60.0/RR;
        const breathholdduration = ihfraction*60.0/RR;
        const timenextbreath = params.timenextbreath;
        const dt = params.dt;
        if (t > timenextbreath - dt/2 & t < timenextbreath + breathduration + breathholdduration) {
            return R*C3 + C2/C;
        }
        else {
             return peep;   
    ...
    function iterate3(t, C1, C2, C3, params) {
        ...
        if (t > timenextbreath - dt/2 & t < timenextbreath + breathduration) {
            return tv/breathduration;
        }
        else if (t > timenextbreath + breathduration - dt/2 & t < timenextbreath + breathduration + breathholdduration) {
            return 0;
        }
        else {
    ...
    let params = { TV: [], PEEP: [], RR: [], IF: [], IHF: [], R: [], C: [], timenextbreath: [], tfinal: [], dt: []};
    
    params.TV = TVslider.value/10.0;
    params.PEEP = PEEPslider.value/10.0;
    params.RR = RRslider.value/10.0;
    params.IF = IFslider.value/100.0;
    params.IHF = IHFslider.value/100.0;
    ...
    let counter = 0;
    while (counter < N) {
        if (ts[counter] > params.timenextbreath + (params.IHF + params.IF)*60.0/params.RR) {
...
TVslider.addEventListener("change", function() {
    TVnum.value = TVslider.value/10;
    dfsolve();    
});

TVnum.addEventListener("change", function() {
    TVslider.value = TVnum.value*10;
    dfsolve();    
});
...
IHFslider.addEventListener("change", function() {
    IHFnum.value = IHFslider.value/100;
    dfsolve();    
});

IHFnum.addEventListener("change", function() {
    IHFslider.value = IHFnum.value*100;
    dfsolve();    
});
